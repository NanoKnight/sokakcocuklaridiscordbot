...
.setColor("GREEN")
    .setFooter("sokakcouklari", client.user.avatarURL);
...
client.on('message', async msg => {
  ...
  if (command === 'çal') {
    ...
    if (!voiceChannel) return msg.channel.sendEmbed(new Discord.RichEmbed()
      .setColor('RANDOM')
    .setDescription(':warning: | Lütfen bir sesli kanala girin'));
    ...
    if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
      ...
      try {
        ...
        try {
         ...
         .setTitle('sokakcouklari | Şarkı Seçimi')
    ...
    if (!serverQueue) return msg.channel.sendEmbed(new Discord.RichEmbed()
     .setColor('RANDOM')
     .setTitle('  **Herhangi Bir Müzik Çalınmamakta **'));                                              
    serverQueue.connection.dispatcher.end('**MÜZİK GEÇİLDİ! :fast_forward: **');
    return undefined;
    ...
    msg.channel.send(`:stop_button: **${serverQueue.songs[0].title}** Müzik Durduruldu`);
    serverQueue.songs = [];
   ...
   .setTitle(`:warning: Şuanki Ses Seviyesi: **${serverQueue.volume}**`)
    ...
    .setColor('RANDOM'));
      return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setColor('RANDOM')
     .setTitle('sokakcouklari | Şarkı Kuyruğu')
    ...
    return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setTitle(":warning: ** | Çalan Müzik Bulunmamakta. :negative_squared_cross_mark:  **")
    .setColor('RANDOM'));
  }
...
function play(guild, song) {
  ...
  .setTitle("**sokakcouklari| 🎙 Müzik Başladı**",`https://cdn.discordapp.com/avatars/473974675194511361/6bb90de9efe9fb80081b185266bb94a6.png?size=2048`)
...
client.on('guildBanAdd' , (guild, user) => {
  let genel = guild.channels.find('name', 'kırmızı-halı');
  if (!genel) return;
  genel.send('**ŞAAAAAK BANLANDIN OROSPU EVLADI !!!** @'+ user.username  );
});
